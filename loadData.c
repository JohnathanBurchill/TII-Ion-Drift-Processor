...
// Adapted from SLIDEM
int getLpInputFilename(const char satelliteLetter, long year, long month,
                       long day, const char *path, char *filename) {
  ...
  long fileVersion;
  while (f != NULL) {
    if ((strlen(f->fts_name) == 59 || strlen(f->fts_name) == 70) &&
        *(f->fts_name + 11) == satelliteLetter &&
        strncmp(f->fts_name + 13, "LP_HM", 5) == 0) {
      char fyear[5] = {0};
      char fmonth[3] = {0};
      char fday[3] = {0};
      char version[5] = {0};
...
int loadLpCalData(ProcessorState *state) {
  ...
  int status = TIICT_OK;
  // Only used for version 0401 or greater, as earlier versions of calibration
  // files did not include potential estimates
  ...
  if (status == TIICT_OK) {
    if (state->nLpRecs < LP_MIN_NUMBER_OF_POTENTIALS) {
      fprintf(state->processingLogFile,
              "%sNot enough (%lu) LP potentials imported.\n", infoHeader,
              state->nLpRecs);
      status = TIICT_NO_LP_HM_DATA;
    } else {
      fprintf(state->processingLogFile,
              "%sLoaded %lu LP potentials, and interpolated them to the TII "
              "times.\n",
...
int getLpData(ProcessorState *state) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    res = getLpInputFilename(state->args.satellite[0], date.tm_year + 1900,
                             date.tm_mon + 1, date.tm_mday, state->args.lpDir,
                             lpFile);
    if (res != 0) {
    ...
    status = loadLpInputs(lpFile, &lpTimes2Hz, &lpVsHg, &lpVsLg, &lpVs, &lpNi,
                          &state->nLpRecs);
    if (status == TIICT_MEMORY) {
      fprintf(state->processingLogFile,
              "%sUnable to allocate memory for LP data. Skipping processing.",
              infoHeader);
      return status;
    }
  ...
  if (state->nLpRecs == 0) {
    fprintf(state->processingLogFile,
            "%sNo LP records found. Skipping processing.", infoHeader);
    return TIICT_NO_LP_HM_DATA;
  }
  ...
  state->lpNi = malloc(sizeof(float) * state->nRecs);
  if (state->lpPhiScHighGain == NULL || state->lpPhiScLowGain == NULL ||
      state->lpPhiSc == NULL || state->lpNi == NULL) {
    fprintf(state->processingLogFile,
            "%sUnable to allocate memory for LP interpolation. Skipping "
            "processing.",
            infoHeader);
    return TIICT_MEMORY;
  }
  ...
  bzero(state->lpNi, sizeof(float) * state->nRecs);

  double *tiiTime = (double *)state->dataBuffers[0];

  interpolate(lpTimes2Hz, lpVsHg, state->nLpRecs, tiiTime, state->nRecs,
              state->lpPhiScHighGain);
  interpolate(lpTimes2Hz, lpVsLg, state->nLpRecs, tiiTime, state->nRecs,
              state->lpPhiScLowGain);
  interpolate(lpTimes2Hz, lpVs, state->nLpRecs, tiiTime, state->nRecs,
              state->lpPhiSc);
  interpolate(lpTimes2Hz, lpNi, state->nLpRecs, tiiTime, state->nRecs,
              state->lpNi);
...
// Adapted from SLIDEM
int loadLpInputs(const char *cdfFile, double **lpTime, double **lpPhiScHighGain,
                 double **lpPhiScLowGain, double **lpPhiSc, double **lpNi,
  ...
  // Check CDF info
  long decoding, encoding, majority, maxrRec, numrVars, maxzRec, numzVars,
      numAttrs, format;
  ...
  status = CDFinquireCDF(cdfId, &numDims, dimSizes, &encoding, &majority,
  ...
  int nVariables = 5;
  char *variables[5] = {"Timestamp", "Vs_hgn", "Vs_lgn", "U_SC", "n"};
  ...
  for (uint8_t i = 0; i < nVariables; i++) {
    ...
    status =
        CDFreadzVarAllByVarID(cdfId, varNum, &numRecs, &dataType, &numElems,
                              &numDims, dimSizes, &recVary, dimVarys, &data);
    if (status != CDF_OK) {
      closeCdf(cdfId);
      CDFdataFree(data);
      return TIICT_CDF_READ;
    ...
    case 3: // U_SC
      ...
      *lpPhiSc = (double *)newMem;
      ...
      break;
    case 4: // n (density from ion current)
      newMem = (double *)realloc(*lpNi, totalBytes);
      if (newMem == NULL)
        return TIICT_MEMORY;
      *lpNi = (double *)newMem;
...
int loadTiiCalData(ProcessorState *state) {
  ...
  for (int8_t i = -1; i < 2; i++) {
    ...
    fprintf(state->processingLogFile,
            "%sLoading calibration data for %04d%02d%02d\n", infoHeader,
            state->args.year, state->args.month, state->args.day);
  ...
  fprintf(state->processingLogFile, "%sNumber of records: %ld\n", infoHeader,
          state->nRecs);
  fprintf(state->processingLogFile,
          "%sLoaded %ld bytes (%ld MB) of calibration data.\n", infoHeader,
          state->memoryAllocated, state->memoryAllocated / 1024 / 1024);
...
void loadTiiCalDataFromDate(const DayType dayType, ProcessorState *state) {
  // c time manipulation: see
  // https://fresh2refresh.com/c-programming/c-time-related-functions/
  struct tm timestructure = {0};
  time_t date;

  // day or month can be outside nominal ranges due to date arithmetic, so make
  // them canonical
  ...
  setCalibrationFileName(state, state->args.year, state->args.month,
                         state->args.day);
  fprintf(state->processingLogFile, "%s from %s\n", infoHeader,
          state->calibrationFileName);
  ...
  if (status != CDF_OK) {
    // Not necessarily an error. For example, some dates will have not
    // calibration data.
  ...
  // Check CDF info
  long numDims, decoding, encoding, majority, maxrRec, numrVars, maxzRec,
      numzVars, numAttrs, format;
  long dimSizes[CDF_MAX_DIMS];

  status = CDFgetFormat(calCdfId, &format);
  status = CDFgetDecoding(calCdfId, &decoding);

  status = CDFinquireCDF(calCdfId, &numDims, dimSizes, &encoding, &majority,
                         &maxrRec, &numrVars, &maxzRec, &numzVars, &numAttrs);
  if (status != CDF_OK) {
    fprintf(state->processingLogFile,
            "%sProblem with calibration file. Skipping this date.\n",
            infoHeader);
    closeCdf(calCdfId);
    return;
  }
  ...
  status =
      CDFgetzVarAllocRecords(calCdfId, CDFgetVarNum(calCdfId, "epoch"), &nRecs);
  if (status != CDF_OK) {
    fprintf(state->processingLogFile,
            "%sProblem with calibration file. Skipping this date.\n",
            infoHeader);
  ...
  if (nRecs < (16 * SECONDS_OF_DATA_REQUIRED_FOR_PROCESSING)) {
    ...
    // TODO: maybe increase this threshold to require a larger number of points
    // each day?
    fprintf(state->processingLogFile,
            "%sFewer than %.0f s of data. Skipping this date.\n", infoHeader,
            (float)SECONDS_OF_DATA_REQUIRED_FOR_PROCESSING);
  ...
  case PREVIOUS_DAY:
    stopRecord = nRecs - 1;
    // Work backward from end of file to find first record greater than or equal
    // to 95 minutes from end of day
    timeReference = computeEPOCH(
        state->args.year, state->args.month, state->args.day + 1, 0, 0,
        -(SECONDS_OF_BOUNDARY_DATA_REQUIRED_FOR_PROCESSING), 0);
    for (startRecord = stopRecord; startRecord >= 0; startRecord--) {
      status =
          CDFgetzVarRecordData(calCdfId, epochNum, startRecord, &recordTime);
      if (status != CDF_OK) {
        fprintf(state->processingLogFile,
                "%sCould not read epoch record from CDF file. Skipping this "
                "calibration date.\n",
                infoHeader);
        closeCdf(calCdfId);
        return;
  ...
  case REQUESTED_DAY:
    ...
    break;
  case NEXT_DAY:
    // Work forward from start of file to find last record less than or equal to
    // 95 minutes from start of day
    startRecord = 0;
    timeReference =
        computeEPOCH(state->args.year, state->args.month, state->args.day, 0, 0,
                     (SECONDS_OF_BOUNDARY_DATA_REQUIRED_FOR_PROCESSING), 0);
    for (stopRecord = startRecord; stopRecord < nRecs; stopRecord++) {
      status =
          CDFgetzVarRecordData(calCdfId, epochNum, stopRecord, &recordTime);
      if (status != CDF_OK) {
        fprintf(state->processingLogFile,
                "%sCould not read epoch record from CDF file. Skipping this "
                "calibration date.\n",
  ...
  default:
    fprintf(state->processingLogFile,
            "%sError: Day type must be one of PREVIOUS_DAY, REQUESTED_DAY, or "
            "NEXT_DAY. Skipping this calibration data.\n",
            infoHeader);
    closeCdf(calCdfId);
    return;
  }
  ...
  if ((dayType == REQUESTED_DAY &&
       nRecs < (16 * SECONDS_OF_DATA_REQUIRED_FOR_PROCESSING)) ||
      ((dayType == PREVIOUS_DAY || dayType == NEXT_DAY) &&
       nRecs < (16 * SECONDS_OF_BOUNDARY_DATA_REQUIRED_FOR_PROCESSING))) {
    // Not enough to do anything useful
    // TODO: maybe increase this threshold to require a larger number of points
    // each day?
    fprintf(
        state->processingLogFile,
        "%sFewer than %.0f s of data meet constraints. Skipping this date.\n",
  ...
  if (nVars != NUM_CAL_VARIABLES) {
    fprintf(state->processingLogFile,
            "%sError: number of calibration variables should be %d. Got %ld. "
            "Skipping this date.\n",
            infoHeader, (uint8_t)NUM_CAL_VARIABLES, numzVars);
    closeCdf(calCdfId);
    return;
  }
  fprintf(state->processingLogFile, "%sChecking calibration file variables...",
          infoHeader);
  for (uint8_t i = 0; i < nVars; i++) {
    ...
    if (status != CDF_OK) {
      ...
      fprintf(state->processingLogFile,
              "%sError reading variable %s. Skipping this date.\n", infoHeader,
              variables[i]);
      closeCdf(calCdfId);
      return;
    } else {
  ...
  for (uint8_t i = 0; i < nVars; i++) {
    ...
    if (varNum < CDF_OK) {
      ...
      fprintf(state->processingLogFile,
              "%sError reading variable ID for %s. Skipping this date.\n",
    ...
    status =
        CDFgetzVarRangeRecordsByVarID(calCdfId, varNum, startRecord, stopRecord,
                                      state->dataBuffers[i] + numBytesPrev);
    if (status != CDF_OK) {

      fprintf(state->processingLogFile,
              "%sError loading data for %s. Skipping this date.\n", infoHeader,
              variables[i]);
      closeCdf(calCdfId);
      return;
    }
...
void setCalibrationFileName(ProcessorState *state, int year, int month,
                            int day) {
  Arguments *a = &state->args;
  snprintf(state->calibrationFileName, CDF_PATHNAME_LEN,
           "%s/%s/%04d/Swarm_%s/%02d/TiiClbr%s_Swarm_%s_%04d_%02d_%02d.cdf",
           a->calDir, a->calVersion, year, a->satellite, month, a->calVersion,
           a->satellite, year, month, day);
...
int checkCalDataAvailability(ProcessorState *state) {
  ...
  if (access(state->calibrationFileName, F_OK) != 0) {
    fprintf(state->processingLogFile,
            "%sCalibration file %s not found. Skipping this date.\n",
  ...
  if (status != CDF_OK) {
    fprintf(state->processingLogFile,
            "%sUnable to open %s. Skipping this date.\n", infoHeader,
            state->calibrationFileName);
    return TIICT_CDF_READ;
  }
  ...
  long nRecords = 0;
  status = CDFgetzVarAllocRecords(calCdfId, CDFgetVarNum(calCdfId, "epoch"),
                                  &nRecords);
  if (status != CDF_OK) {
  ...
  if (nRecords < (16 * SECONDS_OF_DATA_REQUIRED_FOR_PROCESSING)) {
    fprintf(state->processingLogFile,
            "%sLess than %.0f s of data available. Skipping this date.\n",
            infoHeader, (float)SECONDS_OF_DATA_REQUIRED_FOR_PROCESSING);
    return TIICT_NOT_ENOUGH_CALIBRATION_RECORDS;
  } else {
    fprintf(state->processingLogFile,
            "%sProcessing %ld calibration records for this date.\n", infoHeader,
            nRecords);
  }
