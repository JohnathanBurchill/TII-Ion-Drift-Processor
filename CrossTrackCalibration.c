...
#include "utilities.h"
#include "CrossTrackCalibration.h"
#include <cdf.h>
...
int main(int argc, char* argv[])
{
    ...
    // args: satellite, year, month, day, calversion, exportVersion
    if (argc != 9)
    {
        fprintf(stdout, "usage: CrossTrackCalibration satLetter year month day calversionString exportVersionString calDir exportDir\n");
        exit(1);
    }
    ...
    // set up info header
    sprintf(infoHeader, "TCT %c%s %04d-%02d-%02d: ", satellite[0], exportVersion, year, month, day);
    fprintf(stdout, "\n%s-------------------------------------------------\n", infoHeader);
    fprintf(stdout, "%sCrossTrackCalibration Version TCT16 0301 20200611\n", infoHeader);
    ...
    if (fitFile == NULL)
    {
        fprintf(stdout, "%sError opening file fit log file:\n  %s\nAborting.\n", infoHeader, fitLogFileName);
        exit(1);
    }
...
void removeOffsetsAndSetFlags(const char* satellite, offset_model_fit_arguments fitargs, long nRecs, uint8_t **dataBuffers, float *viErrors, uint16_t *flags, uint32_t *fitInfo, FILE* fitFile)
{
    ...
    if (keepRunning)
    {
        ...
        int8_t firstDirection = (dlatfirst > 0) - (dlatfirst < 0);
        int8_t secondDirection = (dlatsecond > 0) - (dlatsecond < 0);
        ...
        for (timeIndex = 0; timeIndex < nRecs; timeIndex++)
        {
            ...
            // fprintf(stdout, "%st = %8.3f, QDLat: %4.1f, MLT: %3.1f, mxh: %3.1f, mxv: %3.1f, myh: %3.1f, myv: %3.1f, \
    vsatx: %5.1f, vsaty: %5.1f, vsatz: %5.1f, lat: %4.1f, lon: %4.1f, rad: %7.1f, vsatn: %5.1f, vsate: %5.1f, vsatc: %5.1f, \
    bc: %6.1f, vbias: %3.1f, vfp: %3.1f\
    \n", seconds, QDLAT(), MLT(), MXH(), MXV(), MYH(), MYV(), VSATX(), VSATY(), VSATZ(), LAT(), LON(), RADIUS(), VSATN(), VSATE(), VSATC(), BC(), VBIAS(), VFP());
            previousQDLat = QDLAT();
