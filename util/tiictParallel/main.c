...
void *runThread(void *a)
{
	...
	sprintf(command, "tiict %s %d %d %d %s %s %s %s > %s/%s%4d%02d%02d.log 2>&1", args->satLetter, args->year, args->month, args->day, args->calVersion, args->exportVersion, args->calDir, args->exportDir, args->exportDir,args->satLetter, args->year, args->month, args->day);
